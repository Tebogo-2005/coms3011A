...
/**
 * Attaches a derived `overdue` boolean to a raw task row.
 * A task is overdue if its due date has passed, it is not archived,
 * and it is not already Complete. Overdue is never stored - always derived.
 */
...
export function updateTask(id, updates, dbPath) {
  ...
  const allowed = ["title", "description", "due_date", "topic", "status"];
  const fieldMap = {
    title: updates.title,
  ...
  for (const col of allowed) {
    if (fieldMap[col] !== undefined) {
      sets.push(`${col} = @${col}`);
      params[col] = fieldMap[col];
    }
  }
